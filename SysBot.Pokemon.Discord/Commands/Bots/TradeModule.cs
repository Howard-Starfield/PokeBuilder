...
public class TradeModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public Task TradeAsyncAttach([Summary("Trade Code")] int code, [Summary("Ignore AutoOT")] bool ignoreAutoOT = false)
    {
        ...
        return ProcessTradeAttachmentAsync(code, sig, Context.User, ignoreAutoOT: ignoreAutoOT, allowBatchAttachments: true);
    }
    ...
    public async Task TradeAsyncAttach([Summary("Ignore AutoOT")] bool ignoreAutoOT = false)
    {
        ...
        {
            await ProcessTradeAttachmentAsync(code, sig, Context.User, ignoreAutoOT: ignoreAutoOT, allowBatchAttachments: true).ConfigureAwait(false);
    ...
    [Alias("bt")]
    [Priority(1)]
    [Summary("Makes the bot trade the attached Pokémon files as one batch using the supplied trade code.")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task BatchTradeAttachAsync([Summary("Trade Code")] int code, [Summary("Ignore AutoOT")] bool ignoreAutoOT = false)
    {
        var sig = Context.User.GetFavor();
        return ProcessBatchTradeAttachmentsAsync(code, sig, Context.User, ignoreAutoOT);
    }

    [Command("batchTrade")]
    [Alias("bt")]
    [Priority(1)]
    [Summary("Makes the bot trade the attached Pokémon files as one batch.")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task BatchTradeAttachAsync([Summary("Ignore AutoOT")] bool ignoreAutoOT = false)
    {
        var userID = Context.User.Id;
        var code = Info.GetRandomTradeCode(userID);
        var sig = Context.User.GetFavor();
    ...
    [Command("batchTrade")]
    [Alias("bt")]
    [Summary("Makes the bot trade multiple Pokémon from Showdown sets separated by ---. Standard users can trade up to 5; favored, VIP/sudo, and owner users use the configured cap or are unlimited when it is 0.")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public async Task BatchTradeAsync([Summary("List of Showdown Sets separated by '---'")][Remainder] string content)
    {
        var batchConfig = SysCord<T>.Runner.Config.Legality;

        // Check if batch trades are allowed
        if (!batchConfig.AllowBatchTrades)
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context,
        ...
        // Standard users stop at five. Favored / sudo / owner users are unlimited when the
        // operator cap is unset, and otherwise use that positive cap. Kept in sync with the HTTP
        // API and file-attachment paths via TradeBatchLimits.
        bool isFavored = Context.User.GetFavor() is RequestSignificance.Favored or RequestSignificance.Owner;
        ...
        {
            string suffix = GetElevatedLimitSuffix(batchConfig.MaxPkmsPerTrade, isFavored);
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                $"You can only process up to {maxTradesAllowed} trades at a time{suffix}. Please reduce the number of trades in your batch.", 5);
            return;
        }
    ...
    private async Task ProcessTradeAttachmentAsync(int code, RequestSignificance sig, SocketUser user,
        bool isHiddenTrade = false, bool ignoreAutoOT = false, bool allowBatchAttachments = false)
    {
        if (allowBatchAttachments && Context.Message.Attachments.Count > 1)
        {
            await ProcessBatchTradeAttachmentsAsync(code, sig, user, ignoreAutoOT).ConfigureAwait(false);
            return;
        }
    ...
    private async Task ProcessBatchTradeAttachmentsAsync(int code, RequestSignificance sig, SocketUser user, bool ignoreAutoOT)
    {
        int attachmentCount = Context.Message.Attachments.Count;
        if (attachmentCount == 0)
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context, "No attachment provided!", 5).ConfigureAwait(false);
            return;
        }
        ...
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                "Batch trades are currently disabled by the bot administrator.", 5).ConfigureAwait(false);
            return;
        }

        bool isFavored = sig is RequestSignificance.Favored or RequestSignificance.Owner;
        int maxTradesAllowed = TradeBatchLimits.GetEffectiveMax(batchConfig.MaxPkmsPerTrade, isFavored);
        if (attachmentCount > maxTradesAllowed)
        {
            string suffix = GetElevatedLimitSuffix(batchConfig.MaxPkmsPerTrade, isFavored);
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                $"You can only process up to {maxTradesAllowed} files at a time{suffix}. Please reduce the number of files in your batch.", 5).ConfigureAwait(false);
            return;
        }
        ...
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                "You already have an existing trade in the queue that cannot be cleared. Please wait until it is processed.", 2).ConfigureAwait(false);
            return;
        }

        var processingMessage = await Context.Channel.SendMessageAsync(
            $"{user.Mention} Processing your batch trade with {attachmentCount} Pokémon files...").ConfigureAwait(false);

        try
        {
            var result = await BatchAttachmentHelpers<T>.ProcessAsync(Context.Message.Attachments).ConfigureAwait(false);
            await processingMessage.DeleteAsync().ConfigureAwait(false);

            if (!result.IsValid)
            {
                await BatchAttachmentHelpers<T>.SendErrorsAsync(Context, result.Errors).ConfigureAwait(false);
                return;
            }

            var pokemon = result.Pokemon.ToList();
            await QueueHelper<T>.AddBatchContainerToQueueAsync(Context, code, user.Username,
                pokemon[0], pokemon, sig, user, pokemon.Count, ignoreAutoOT).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            try
            ...
            LogUtil.LogSafe(ex, nameof(ProcessBatchTradeAttachmentsAsync));
            await Context.Channel.SendMessageAsync(
                $"{user.Mention} An error occurred while processing your attached batch. Please try again.").ConfigureAwait(false);
        }
    }

    private static string GetElevatedLimitSuffix(int configuredMax, bool isFavored)
    {
        if (isFavored)
            return "";
        if (configuredMax <= 0)
            return " (favored/VIP/owner users have no bot-side limit)";
        if (configuredMax > TradeBatchLimits.DefaultMaxStandard)
            return $" (favored/VIP/owner users: up to {configuredMax})";
        return "";
    }
