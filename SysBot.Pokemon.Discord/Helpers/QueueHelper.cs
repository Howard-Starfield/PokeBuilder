...
public static class QueueHelper<T> where T : PKM, new()
{
    ...
    private static bool HasUnlimitedTradeLimit(RequestSignificance sig) =>
        sig is RequestSignificance.Favored or RequestSignificance.Owner;

    private static TradeRateLimitDecision? TryReserveTradeLimit(ulong userId, int requestedCount, RequestSignificance sig)
    {
        var cfg = SysCord<T>.Runner.Config.Trade.TradeConfiguration;
        if (!cfg.EnableHourlyTradeLimit || HasUnlimitedTradeLimit(sig))
            return null;
    ...
    private static async Task SendTradeLimitReachedEmbedAsync(ISocketMessageChannel channel, SocketUser trader, TradeRateLimitDecision decision)
    {
        string description;
        if (decision.FailureReason == "request_exceeds_limit")
        {
            description =
                $"{trader.Mention}, this request contains **{decision.RequestedCount}** Pokémon, but free users can only complete **{decision.Limit}** Pokémon every **{decision.WindowMinutes} minutes**.";
        }
        else if (decision.PendingCount > 0)
        {
            description =
                $"{trader.Mention}, you already have **{decision.PendingCount}** Pokémon reserved in active queued trades and **{decision.UsedCount}/{decision.Limit}** completed trades in the current window.\nFinish or cancel your queued trades before requesting more.";
        }
        else if (decision.RetryAtUnixSeconds.HasValue)
        {
            description =
                $"{trader.Mention}, you've reached **{decision.UsedCount}/{decision.Limit}** completed trades in the last **{decision.WindowMinutes} minutes**.\nNext free trade available <t:{decision.RetryAtUnixSeconds.Value}:R>.";
        }
        else
        {
            description =
                $"{trader.Mention}, you've reached the current free trade limit of **{decision.Limit}** Pokémon per **{decision.WindowMinutes} minutes**.";
        }

        var embed = new EmbedBuilder()
            .WithColor(DiscordColor.Orange)
            .WithTitle("Hourly Trade Limit Reached")
    ...
    private static async Task SendVipBypassEmbedAsync(ISocketMessageChannel channel, SocketUser trader, int bypassedCount)
    {
        string description = bypassedCount > 0
            ? $"{trader.Mention}, thanks for being a VIP. You bypassed **{bypassedCount}** queued request(s)."
            : $"{trader.Mention}, thanks for being a VIP. Priority access is active for your request.";

        var embed = new EmbedBuilder()
            .WithColor(DiscordColor.Gold)
            .WithTitle("VIP Priority Applied")
            .WithDescription(description)
            .WithTimestamp(DateTimeOffset.Now)
            .Build();
    ...
    {
        if (!TryRegisterQueueSubmission(context.Message.Id))
        {
            LogUtil.LogInfo(nameof(QueueHelper<T>),
                $"Duplicate queue submission ignored. MessageId={context.Message.Id}, User={context.User.Username}, Code={code}, Routine={routine}, Type={type}");
            return;
        }
        ...
        try
        {
            ...
            // The initial queue update already contains the link code for standard trades.
            // Only LGPE still needs its pictocode image as a separate DM.
            if (result.Success && !isBatchTrade && trade is PB7 && lgcode != null)
            {
                var (thefile, lgcodeembed) = CreateLGLinkCodeSpriteEmbed(lgcode);
                await trader.SendDirectFileAsync(thefile, "Your trade code will be.", lgcodeembed).ConfigureAwait(false);
            }
        }
        ...
        List<Pictocodes>? lgcode = null, bool ignoreAutoOT = false, bool setEdited = false, bool isNonNative = false)
    {
        ...
        var user = trader;
        var userID = user.Id;
        var limitDecision = TryReserveTradeLimit(userID, 1, sig);
        if (limitDecision is { Allowed: false })
        {
            await SendTradeLimitReachedEmbedAsync(context.Channel, trader, limitDecision).ConfigureAwait(false);
            return new TradeQueueResult(false);
        }

        string? reservationId = limitDecision?.ReservationId;
        var name = user.Username;
        var trainer = new PokeTradeTrainerInfo(trainerName, userID);
        var discordNotifier = new DiscordTradeNotifier<T>(pk, trainer, code, trader, batchTradeNumber, totalBatchTrades,
            isMysteryEgg, lgcode: lgcode!);
        IPokeTradeNotifier<T> notifier = reservationId is null
            ? discordNotifier
            : new RateLimitedTradeNotifier<T>(discordNotifier, reservationId);
        ...
        var preAddEntryCount = Info.GetTotalEntryCount();
        var added = Info.AddToTradeQueue(trade, userID, false, isSudo);
        if (added != QueueResultAdd.Added && reservationId is not null)
        ...
        if (added == QueueResultAdd.NotAllowedItem)
        {
            ...
            await context.Channel.SendMessageAsync($"{trader.Mention} - Trade blocked: the held item '{itemName}' cannot be traded in LZA.").ConfigureAwait(false);
            return new TradeQueueResult(false);
        }
        ...
        try
        {
            ...
            embedData.EmbedImageUrl = isMysteryEgg ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/mysteryegg3.png" :
                                       type == PokeRoutineType.Dump ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/AltBallImg/128x128/dumpball.png" :
                                       type == PokeRoutineType.Clone ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/clonepod.png" :
                                       type == PokeRoutineType.SeedCheck ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/specialrequest.png" :
                                       type == PokeRoutineType.FixOT ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/AltBallImg/128x128/rocketball.png" :
                                       embedImageUrl;
            ...
            var bypassedCount = sig == RequestSignificance.Favored
                ? Math.Max(0, (preAddEntryCount + 1) - Info.GetEntryPosition(userID, uniqueTradeID))
                : 0;
            ...
            if (pk is IHomeTrack homeTrack)
            {
                ...
                {
                    embedBuilder.Footer.IconUrl = "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/exclamation.gif";
            ...
            if (sig == RequestSignificance.Favored)
                await SendVipBypassEmbedAsync(context.Channel, trader, bypassedCount).ConfigureAwait(false);
        }
        catch (HttpException ex)
        {
    ...
    public static async Task AddBatchContainerToQueueAsync(SocketCommandContext context, int code, string trainer, T firstTrade, List<T> allTrades, RequestSignificance sig, SocketUser trader, int totalBatchTrades, bool ignoreAutoOT = false)
    {
        ...
        var limitDecision = TryReserveTradeLimit(userID, totalBatchTrades, sig);
        if (limitDecision is { Allowed: false })
        {
            await SendTradeLimitReachedEmbedAsync(context.Channel, trader, limitDecision).ConfigureAwait(false);
            return;
        ...
        var discordNotifier = new DiscordTradeNotifier<T>(firstTrade, trainer_info, code, trader, 1, totalBatchTrades, false, lgcode: []);
        IPokeTradeNotifier<T> notifier = reservationId is null
            ? discordNotifier
            : new RateLimitedTradeNotifier<T>(discordNotifier, reservationId);
        ...
        var detail = new PokeTradeDetail<T>(firstTrade, trainer_info, notifier, PokeTradeType.Batch, code,
            sig == RequestSignificance.Favored, null, 1, totalBatchTrades, false, uniqueTradeID, ignoreAutoOT)
        {
    ...
    private static bool TryRegisterQueueSubmission(ulong messageId)
    {
        lock (QueueSubmissionLock)
        {
            var cutoff = DateTime.UtcNow.AddMinutes(-10);
            var expired = RecentQueueMessageIds
                .Where(z => z.Value < cutoff)
                .Select(z => z.Key)
                .ToArray();

            foreach (var key in expired)
                RecentQueueMessageIds.Remove(key);

            if (RecentQueueMessageIds.ContainsKey(messageId))
                return false;

            RecentQueueMessageIds[messageId] = DateTime.UtcNow;
            return true;
        }
    }
...
#pragma warning restore CA1416 // Validate platform compatibility
    ...
    private static async Task<(string, DiscordColor)> PrepareEmbedDetails(T pk)
    {
        ...
        string ballImgUrl = $"https://raw.githubusercontent.com/Howard-Starfield/sprites/main/AltBallImg/20x20/{ballName}.png";
