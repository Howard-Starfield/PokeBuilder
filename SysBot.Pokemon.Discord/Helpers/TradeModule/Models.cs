...
public sealed record ShowdownProcessingPolicy(
    byte GenerateLanguage,
    int DefaultHeldItem,
    bool EnableSpamCheck,
    GameVersion? TrainerVersion = null);
...
public class ProcessedPokemonResult<T> where T : PKM, new()
{
    ...
    public string? LegalizationResult { get; set; }
    public string? TrainerVersion { get; set; }
    public IEncounterable? SelectedEncounter { get; init; }
