...
internal static class DiscordMessageDeduplicator
{
    private static readonly object Sync = new();
    private static readonly Dictionary<ulong, DateTime> RecentMessageIds = [];

    public static bool TryRegister(ulong messageId)
    {
        lock (Sync)
        {
            var cutoff = DateTime.UtcNow.AddMinutes(-10);
            var expired = RecentMessageIds
                .Where(z => z.Value < cutoff)
                .Select(z => z.Key)
                .ToArray();

            foreach (var key in expired)
                RecentMessageIds.Remove(key);

            if (RecentMessageIds.ContainsKey(messageId))
                return false;

            RecentMessageIds[messageId] = DateTime.UtcNow;
            return true;
        }
    }
}
...
public sealed class SysCord<T> where T : PKM, new()
{
    ...
    {
        _channelStatusUpdate.SetDesired(status);
    ...
    private async Task SendBotStatusAnnouncementAsync(string status, EmbedColorOption color)
    {
        ...
        var thumbnailUrl = status == "Online"
            ? "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/botgo.png"
            : "https://raw.githubusercontent.com/Howard-Starfield/sprites/main/botstop.png";
        ...
        var statusUrl = SysCordSettings.Settings.BotStatusUrl;
        if (Uri.TryCreate(statusUrl, UriKind.Absolute, out var uri) &&
            (uri.Scheme == Uri.UriSchemeHttps || uri.Scheme == Uri.UriSchemeHttp))
        {
            embedBuilder.WithUrl(uri.AbsoluteUri);
        }
    ...
    public async Task InitCommands()
    {
        ...
        var moduleTypes = assembly.DefinedTypes
            .Where(z => z.IsSubclassOf(moduleBaseType) && !z.IsAbstract)
            .ToList();

        foreach (var moduleType in moduleTypes)
        {
            var concreteType = moduleType.IsGenericType
                ? moduleType.MakeGenericType(typeof(T))
                : moduleType.AsType();

            await _commands.AddModuleAsync(concreteType, _services).ConfigureAwait(false);
        }
        ...
        var tradeCommandCount = _commands.Commands.Count(c =>
            c.Aliases.Any(a => a.Equals("trade", StringComparison.OrdinalIgnoreCase) ||
                               a.Equals("t", StringComparison.OrdinalIgnoreCase)));
        await Log(new LogMessage(LogSeverity.Info, "InitCommands",
            $"Registered {modules.Count} modules and {tradeCommandCount} trade command overload(s) for {typeof(T).Name}.")).ConfigureAwait(false);
    ...
    private async Task HandleMessageAsync(SocketMessage arg)
    {
        ...
        try
        {
            ...
            if (!DiscordMessageDeduplicator.TryRegister(msg.Id))
            {
                await Log(new LogMessage(
                    LogSeverity.Warning,
                    "Command",
                    $"Duplicate Discord message ignored. MessageId={msg.Id}, Author={msg.Author.Username}, Channel={msg.Channel.Name}")).ConfigureAwait(false);
                return;
            }
    ...
    private async Task<bool> TryHandleCommandAsync(SocketUserMessage msg, SocketCommandContext context, int pos)
    {
        try
        {
            ...
            await Log(new LogMessage(LogSeverity.Info, "Command", $"Executing command {msg.Id} from {guild}#{msg.Channel.Name}:@{msg.Author.Username}. Content: {msg}")).ConfigureAwait(false);
