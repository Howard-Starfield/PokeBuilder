...
public class QueueTests
{
    ...
    private static void TestFavor<T>() where T : PKM, new()
    {
        ...
        // Configure favoritism: Multiply=0.4 means priority users go after 40% of regular users
        var f = settings.Favoritism;
        f.Mode = FavoredMode.Multiply;
        f.Multiply = 0.4f;
        ...
        // With 100 regular users and Multiply=0.4, priority users go after 40 users
        int expectedPosition = (int)Math.Ceiling(count * f.Multiply);
