...
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/Howard-Starfield/PokeBuilder/total?style=flat-square&logoColor=Red&color=red)
...
|---------|---------|-------------|--------|-------------|
| `trade` | `t` | Trade a Pokémon from a Showdown set or attached file; multiple attached files create a batch | `<prefix>trade [code] <showdown_set>` or attach file(s) | Trade Role |
| `hidetrade` | `ht` | Trade without showing embed details | `.hidetrade [code] <showdown_set>` or attach file | Trade Role |
| `batchTrade` | `bt` | Trade multiple Pokémon from Showdown sets or attached files | `<prefix>bt <sets_separated_by_--->` or attach file(s) | Trade Role |
...
- **Command Prefix**: Replace `<prefix>` with the configured Discord command prefix, such as `.`, `$`, or `!`.
- **Batch Trading**: Separate multiple Showdown sets with `---`, or attach `.pk*` files to either `batchTrade` / `bt` or `trade` / `t`.
- **Batch Limits**: Standard users can request up to 5 Pokémon. Favored, VIP/sudo, and owner users are unlimited unless the operator sets a positive `MaxPkmsPerTrade` cap under `Legality`, immediately below `Allow Batch Trades`.
- **File Support**: One attached file uses the existing single-trade flow; two or more attached files create one ordered batch.
