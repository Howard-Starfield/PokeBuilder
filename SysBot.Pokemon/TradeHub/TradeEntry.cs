...
public sealed record TradeEntry<T>(PokeTradeDetail<T> Trade, ulong UserID, PokeRoutineType Type, string Username, int UniqueTradeID = 0)
    where T : PKM, new()
{
    /// <summary>
    /// Batch trade tracking
    /// </summary>
    public int BatchTradeNumber => Trade.BatchTradeNumber;
    public int TotalBatchTrades => Trade.TotalBatchTrades;
