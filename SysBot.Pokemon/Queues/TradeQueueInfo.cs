    ...
    where T : PKM, new()
{
    ...
    public void ClearAllQueues()
    {
        lock (_sync)
        {
            foreach (var trade in UsersInQueue)
                TradeRateLimitService.Instance.ReleaseReservation(trade.Trade);
    ...
    public bool Remove(TradeEntry<T> detail)
    {
        lock (_sync)
        {
            ...
            var removed = UsersInQueue.Remove(detail);
            if (removed)
                TradeRateLimitService.Instance.ReleaseReservation(detail.Trade);
            return removed;
        }
    }
    ...
    public int GetEntryPosition(ulong uid, int uniqueTradeID)
    {
        lock (_sync)
        {
            var allTrades = Hub.Queues.AllQueues
                .SelectMany(q => q.Queue.Select(x => x.Value))
                .GroupBy(z => new { z.Trainer.ID, z.UniqueTradeID })
                .Select(g => g.First())
                .ToList();
            ...
            var queuedIndex = UsersInQueue.FindIndex(z => z.UserID == uid && z.UniqueTradeID == uniqueTradeID);
            return queuedIndex >= 0 ? queuedIndex + 1 : -1;
        }
    }
