...
public class TradeSettings : IBotStateSettings, ICountSettings
{
    ...
    public class TradeSettingsCategory
    {
        ...
        [Category(TradeConfig), Description("When enabled, non-favored users are limited to a certain number of completed trades within the rolling trade limit window."), DisplayName("Enable Hourly Trade Limit")]
        public bool EnableHourlyTradeLimit { get; set; }

        [Category(TradeConfig), Description("Maximum number of successfully completed Pokémon trades allowed for non-favored users within the rolling trade limit window."), DisplayName("Free Trade Limit Per Hour")]
        public int FreeTradeLimitPerHour { get; set; } = 5;

        [Category(TradeConfig), Description("Rolling time window in minutes used for the free trade limit."), DisplayName("Trade Limit Window (minutes)")]
        public int TradeLimitWindowMinutes { get; set; } = 60;
        ...
        [Browsable(false)]
        public int MaxPkmsPerTrade { get; set; } = 0;
    ...
    // Pass-through properties for backward compatibility with game-specific trade bots
    [Browsable(false)] public int TradeWaitTime => TradeConfiguration.TradeWaitTime;
    [Browsable(false)] public int MaxTradeConfirmTime => TradeConfiguration.MaxTradeConfirmTime;
    [Browsable(false)] public int MaxDumpsPerTrade => TradeConfiguration.MaxDumpsPerTrade;
    [Browsable(false)] public int MaxDumpTradeTime => TradeConfiguration.MaxDumpTradeTime;
    [Browsable(false)] public bool DumpTradeLegalityCheck => TradeConfiguration.DumpTradeLegalityCheck;
    [Browsable(false)] public bool DisallowTradeEvolve => TradeConfiguration.DisallowTradeEvolve;
    [Browsable(false)] public int MaxPkmsPerTrade => TradeConfiguration.MaxPkmsPerTrade;
    [Browsable(false)] public bool AllowBatchTrades => TradeConfiguration.AllowBatchTrades;
    [Browsable(false)] public int CompletedTrades => CountStatsSettings.CompletedTrades;

    public void AddCompletedTrade() => CountStatsSettings.AddCompletedTrade();
    public void AddCompletedSeedCheck() => CountStatsSettings.AddCompletedSeedCheck();
    public void AddCompletedSurprise() => CountStatsSettings.AddCompletedSurprise();
    public void AddCompletedDistribution() => CountStatsSettings.AddCompletedDistribution();
    public void AddCompletedDumps() => CountStatsSettings.AddCompletedDumps();
    public void AddCompletedClones() => CountStatsSettings.AddCompletedClones();
    public void AddCompletedFixOTs() => CountStatsSettings.AddCompletedFixOTs();
