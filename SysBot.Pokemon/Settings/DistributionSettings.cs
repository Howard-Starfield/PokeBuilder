...
public class DistributionSettings : ISynchronizationSetting
{
    ...
    // PokeBot: tracks current ProgramMode to allow mode-specific UI behavior
    [Browsable(false)]
    public ProgramMode CurrentMode { get; set; }
    ...
    // LGPE Distribution Trade Pictocodes
    [Category(Distribute), Description("[LGPE] First code for Let's Go distribution trades.")]
    public Pictocodes LGPECode1 { get; set; } = Pictocodes.Pikachu;

    [Category(Distribute), Description("[LGPE] Second code for Let's Go distribution trades.")]
    public Pictocodes LGPECode2 { get; set; } = Pictocodes.Pikachu;

    [Category(Distribute), Description("[LGPE] Third code for Let's Go distribution trades.")]
