...
public class LegalitySettings
{
    ...
    [Category(Generate), Description("Allow users to submit further customization with Batch Editor commands."), DisplayName("Allow Batch Editor Commands")]
    public bool AllowBatchCommands { get; set; }

    [Category(Generate), Description("Toggle to allow or disallow batch trades from Discord and the web API."), DisplayName("Allow Batch Trades")]
    public bool AllowBatchTrades { get; set; } = true;

    [Category(Generate), Description("Optional operator cap for a batch. Use 0 or a negative value for no operator cap. Standard users are still limited to 5 Pokémon; favored, VIP/sudo, and owner users are unlimited when no operator cap is set."), DisplayName("Maximum Pokémon per Batch (0 = Unlimited Elevated)")]
    ...
    [Category(Misc), Description("When enabled, the bot will apply the trade partner's trainer info to the traded Pokémon.")]
    public bool UseTradePartnerInfo { get; set; } = true;

    [Category(Misc), Description("Disallow non-native Pokémon from being traded.")]
    public bool DisallowNonNatives { get; set; }

    [Category(Misc), Description("Disallow Pokémon with HOME tracker data from being traded.")]
    public bool DisallowTracked { get; set; }
}
