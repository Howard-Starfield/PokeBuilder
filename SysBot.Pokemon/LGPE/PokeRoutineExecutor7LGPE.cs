...
public abstract class PokeRoutineExecutor7LGPE : PokeRoutineExecutor<PB7>
{
    ...
    public async Task WriteBoxPokemon(PB7 pk, int box, int slot, CancellationToken token)
    {
        ...
        await Connection.WriteBytesAsync(data.AsSpan(0, StoredLength).ToArray(), (uint)slotofs, token);
        await Connection.WriteBytesAsync(data.AsSpan(StoredLength).ToArray(), (uint)(slotofs + (ulong)StoredLength + 0x70), token);
    }
