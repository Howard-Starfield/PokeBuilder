...
public static class AutoLegalityWrapper
{
    ...
    public static PKM GetLegal(this ITrainerInfo sav, IBattleTemplate set, out string res)
        => sav.GetLegal(set, out res, out _);

    public static PKM GetLegal(this ITrainerInfo sav, IBattleTemplate set, out string res, out IEncounterable? selectedEncounter)
    {
        var result = sav.GetLegalFromSet(set);
        return Unwrap(result, out res, out selectedEncounter);
    }

    public static PKM GetLegal(
        this ITrainerInfo sav,
        IBattleTemplate set,
        out string res,
        out IEncounterable? selectedEncounter,
        Func<IEncounterable, bool> encounterSelector)
    {
        var result = sav.GetLegalFromSet(set, encounterSelector);
        return Unwrap(result, out res, out selectedEncounter);
    }

    private static PKM Unwrap(
        APILegality.AsyncLegalizationResult result,
        out string res,
        out IEncounterable? selectedEncounter)
    {
        selectedEncounter = result.SelectedEncounter;
