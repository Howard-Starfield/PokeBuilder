...
public partial class BotServer(Main mainForm, int port = 8080, int tcpPort = 8081) : IDisposable
{
    ...
    private SupabasePoller? _supabasePoller;
    private SupabaseClient? _supabaseClient;

    public SupabaseClient? SupabaseClientInstance => _supabaseClient;
    ...
    private async Task HandleRequest(HttpListenerContext context)
    {
        ...
        try
        {
            ...
            {
                var wts = _mainForm.Config?.Hub?.WebTrade;
                if (wts?.Mode == WebTradeMode.Tunnel && !request.IsLocal)
                {
                    var p = request.Url?.LocalPath ?? "";
                    bool isTradeRoute = p == "/api/trade" || p == "/api/trade/batch"
                        || p == "/api/status"
                        || p.StartsWith("/api/queue/", StringComparison.OrdinalIgnoreCase);
                    if (!isTradeRoute)
                    {
    ...
    {
        var settings = _mainForm.Config?.Hub?.WebTrade;
        var allowedOrigins = (settings?.AllowedOrigins ?? "http://localhost:3000")
            .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

        var requestOrigin = request.Headers["Origin"];
        if (requestOrigin != null && allowedOrigins.Any(o => string.Equals(o, requestOrigin, StringComparison.OrdinalIgnoreCase)))
        {
    ...
    private async Task<(int statusCode, object? content, string contentType)> ProcessRequestAsync(HttpListenerRequest request)
    {
        ...
        // Tunnel mode: block non-trade routes from external requests, require JWT
        if (settings?.Mode == WebTradeMode.Tunnel && !request.IsLocal)
        {
            // Only allow trade API routes externally
            bool isTradeRoute = path == "/api/trade"
                || path == "/api/trade/batch"
                || path == "/api/status"
                || path.StartsWith("/api/queue/", StringComparison.OrdinalIgnoreCase);

            if (!isTradeRoute)
                return (403, "{\"error\":\"Access denied\"}", "application/json");

            var authHeader = request.Headers["Authorization"];
            if (string.IsNullOrEmpty(authHeader) || !authHeader.StartsWith("Bearer "))
                return (401, "{\"error\":\"Missing authorization token\"}", "application/json");
            var token = authHeader["Bearer ".Length..];
            if (!JwtValidator.Validate(token, settings.SupabaseJwtSecret))
                return (401, "{\"error\":\"Invalid or expired token\"}", "application/json");
        }
        ...
        return path switch
        {
            ...
            // Trade API routes
            var p when p == "/api/trade" && request.HttpMethod == "POST" =>
                await TradeApiHandler.HandlePostTrade(request, _mainForm.Runner),
            var p when p == "/api/trade/batch" && request.HttpMethod == "POST" =>
                await TradeApiHandler.HandlePostBatchTrade(request, _mainForm.Runner),
            "/api/status" => TradeApiHandler.HandleGetStatus(_mainForm.Runner),
            var p when p.StartsWith("/api/queue/", StringComparison.OrdinalIgnoreCase) && request.HttpMethod == "GET" =>
                TradeApiHandler.HandleGetQueueStatus(p["/api/queue/".Length..]),
            var p when p.StartsWith("/api/queue/", StringComparison.OrdinalIgnoreCase) && request.HttpMethod == "DELETE" =>
                TradeApiHandler.HandleDeleteQueue(p["/api/queue/".Length..]),
    ...
    public void InitializeSupabaseIfNeeded(IPokeBotRunner runner)
    {
        var settings = _mainForm.Config?.Hub?.WebTrade;
        if (settings == null) return;

        // Stop and clean up any existing instance before re-initializing
        StopSupabasePoller();

        if (settings.Mode == WebTradeMode.Supabase || settings.Mode == WebTradeMode.Tunnel)
        {
            if (string.IsNullOrEmpty(settings.SupabaseUrl) || string.IsNullOrEmpty(settings.SupabaseServiceKey))
            {
                LogUtil.LogInfo("BotServer", $"{settings.Mode} mode: Supabase URL/Key not configured. Skipping.");
                return;
            }

            _supabaseClient = new SupabaseClient(settings.SupabaseUrl, settings.SupabaseServiceKey);

            // Only start poller in Supabase mode
            if (settings.Mode == WebTradeMode.Supabase)
            {
                _supabasePoller = new SupabasePoller(_supabaseClient, runner, settings);
                _supabasePoller.Start();
            }
        }
    }

    public void StopSupabasePoller()
    {
        _supabasePoller?.Stop();
        _supabasePoller?.Dispose();
        _supabasePoller = null;
